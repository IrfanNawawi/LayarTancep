 ...
 * heycoding@gmail.com
 */
class GetPopularMovieUseCase(private val repository: HomeRepository, dispatcher: CoroutineDispatcher) : BaseUseCase<Nothing, List<HomeUiItem>>(dispatcher) {
                ...
                doOnSuccess = { result ->
                    ...
                    result.payload?.data?.let { homeData ->
                        homeData.forEach { movie->
                            data.add(HomeUiItem.PopularSectionItem(PopularMapper.toViewParam(movie)))
                        }
                    }
